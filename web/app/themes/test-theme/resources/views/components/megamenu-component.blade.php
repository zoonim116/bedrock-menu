    ...
    <nav class="nav-primary border-solid border-t border-t-slate-600 relative" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
        <div class="container pt-2">
        <div id="product-menu" class="inline-flex items-center gap-2 pb-2"><svg width="15" height="13" viewBox="0 0 15 13" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {{__('Products') }}
            <div class="megamenu bg-white text-indigo-950 max-h-0 top-full transition-all overflow-hidden absolute left-0 w-full">
                    ...
                    @foreach($childElements as $key => $subMenu)
                      <ul data-parent-id="slug_id_{{$key}}" @class([
                        ...
                        @foreach($subMenu as $subMenuItem)
                          <li>
                            <a href="{{$subMenuItem->url}}" class="group menu-item">
                              {{$subMenuItem->title}}
                              <div class="category-preview">
                                @if($subMenuItem->image)
                                  <img src="{{$subMenuItem->image->sizes['medium_large'] }}" alt="" class="hidden group-hover:flex" />
                                @endif
                              </div>
                            </a>
                          </li>
